<template>
  <div>
    <div>
      <div class="searchBox">
        <el-input v-model="equipmentName" placeholder="请输入维修项目名称" style="width: 200px"
                  @keyup.enter.native="loadPost"></el-input>
        <el-date-picker type="date" placeholder="选择起始时间" v-model="lastMaintenanceStartDate" style="width: 150px;"></el-date-picker>
        <el-date-picker type="date" placeholder="选择结束时间" v-model="lastMaintenanceEndDate" style="width: 150px;"></el-date-picker>
    ...
    >
      ...
      <el-table-column prop="equipmentName" label="维修项目" >
      </el-table-column>
      <el-table-column prop="lastMaintenance" label="最近一次维修时间">
      </el-table-column>
    ...
    <div>
      ...
      >
        <el-form ref="form" :rules="rules" :model="form" label-width="80px">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="最近一次维修时间">
                <el-date-picker type="date" placeholder="选择日期" v-model="form.lastMaintenance" style="width: 100%;"></el-date-picker>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <div>
      ...
      >
        <el-form ref="form" :rules="rules" :model="editForm" label-width="80px">
          <el-form-item label="维修项目" prop="equipmentName">
            <el-col :span="20">
              <el-input v-model="editForm.equipmentName">{{}}</el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="最近一次维修时间" style="width: 167px">
            <el-date-picker type="date" placeholder="选择日期" v-model="editForm.lastMaintenance" ></el-date-picker>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods:{
    //导出Excel表
    downloadExcel(){
      this.$axios.post(this.$httpUrl + '/equipment/exportAll', {
        param: {
    ...
    del(){
      ...
      {
        this.delID.push(this.multipleSelection[i].equipmentId)
      }
    },
    ...
    delAll(){
      ...
      {
        this.$axios.get(this.$httpUrl + '/equipment/delete?id='+this.delID[i], ).then(res=>{
          console.log(res);
    ...
    //添加记录
    doSave(){
      this.$axios.post(this.$httpUrl + '/equipment/saveEquipment',this.form).then(res=>{
        console.log(res);
    ...
    //修改记录
    doMod(){
      this.$axios.post(this.$httpUrl + '/equipment/update',
          {
            id: this.editForm.equipmentId,
            lastMaintenance:this.editForm.lastMaintenance,
            name: this.editForm.equipmentName,
    ...
    //加载列表
    loadPost(){
      this.$axios.post(this.$httpUrl + '/equipment/listPage', {
