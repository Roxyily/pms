...
public class EquipmentController {
    ...
    @ApiOperation(value = "导出分页查询的设备信息到Excel文件", notes = "在验证用户登录状态后，根据查询参数进行分页查询设备信息，并将数据导出到Excel文件")
    ...
    public void exportPage(@RequestBody QueryPageParam query, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<EquipmentPropertyDTO> dataList = new ArrayList<>();
        if (userId != null) {
            // 用户已登录，返回用户信息
            User user = userService.getById(Integer.valueOf(userId));
            Map<String, Object> params = new HashMap<>();
            ...
            IPage<EquipmentPropertyDTO> result = equipmentService.getEquipmentsWithProperty(page, params);
        ...
        String fileName = URLEncoder.encode("equipments.xlsx", "UTF-8");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName);

        // 创建一个ExcelWriter
        ExcelWriter excelWriter = EasyExcel.write(response.getOutputStream(), EquipmentPropertyDTO.class).build();
    ...
    @ApiOperation(value = "导出所有设备信息到Excel文件", notes = "在验证用户登录状态后，查询所有满足条件的设备信息，并将数据导出到Excel文件")
